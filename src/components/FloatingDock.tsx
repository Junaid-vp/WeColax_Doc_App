  ...
  return (
    <div className="fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-[calc(100vw-2rem)] sm:max-w-max px-2">
      <motion.div 
        ...
        className="flex items-center gap-1 sm:gap-2 p-1.5 sm:p-2 rounded-2xl bg-white/70 backdrop-blur-2xl border border-slate-200 shadow-xl overflow-x-auto no-scrollbar justify-center"
      >
        {links.map((link) => {
          ...
          return (
            <Link key={link.href} href={link.href} className="shrink-0">
              <motion.div
                ...
                whileTap={{ scale: 0.9 }}
                className={`relative p-2.5 sm:p-3 rounded-xl flex items-center justify-center transition-colors group ${
        ...
        <div className="w-[1px] h-6 sm:h-8 bg-slate-200 mx-1 shrink-0" />
